...
import { formatShippingAddressText } from "@/lib/shipping-address";
...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void UserService.getCurrentProfile()
      .then((me) => {
        if (cancelled) return;
        const savedName =
          me.shippingRecipientName?.trim() ||
          [me.firstName, me.lastName].filter(Boolean).join(" ").trim();
        const savedPhone = me.phone?.trim() ?? "";
        const savedAddress = me.shippingAddress?.trim() ?? "";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const shippingPayload = {
      recipientName: name,
      recipientPhone: phoneTrim,
      recipientAddress: addr,
      ...(note.trim() ? { shippingNote: note.trim() } : {}),
    };
    const shippingAddress = formatShippingAddressText(shippingPayload);

    setSubmitting(true);
    try {
      const { order, paymentRedirectUrl } = await orderService.createOrder({
        items: items.map((row) => ({
          ...
          >
            ...
            <div className="border-t border-gray-100 pt-6">
              <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
                ...
                {showSavedShipping ? (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-md border border-orange-200 bg-orange-50 px-3 py-1.5 text-sm font-medium text-orange-700 transition hover:bg-orange-100"
                  >
              ...
              {!profileLoaded ? (
                ...
                <div className="mb-4 rounded-lg border border-green-100 bg-green-50/80 p-4 text-sm text-gray-800">
                  <p className="mb-3 font-medium text-green-900">
                    Sử dụng thông tin giao hàng đã lưu từ lần đặt trước
                  </p>
                  <dl className="space-y-2">
                    ...
                    <div>
                      <dt className="text-gray-500">Địa chỉ giao hàng</dt>
              ...
              <div className={showSavedShipping ? "" : "mt-0"}>
                <label
                  htmlFor="note"
                  className="mb-1 block text-sm font-medium text-gray-700"
                >
