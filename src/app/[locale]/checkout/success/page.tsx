...
function SuccessContent() {
  ...
  const gatewayResultCode = searchParams.get("resultCode");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        if (stripeSessionId) {
          await paymentGatewayService.verifyStripeSession(
            stripeSessionId,
            orderId,
          );
        }
        if (paypalToken) {
          await paymentGatewayService.capturePaypal(paypalToken, orderId);
        }
  ...
  }, [orderId, stripeSessionId, paypalToken]);
  ...
  return (
    <>
      ...
      {gatewayResultCode && gatewayResultCode !== "0" ? (
        <p className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          Cổng thanh toán báo chưa hoàn tất (mã {gatewayResultCode}). Liên hệ
          shop nếu đã bị trừ tiền.
        </p>
      ) : null}
      <p className="text-gray-600 mb-6">
        Cảm ơn bạn đã mua hàng.
        {isOnlineGatewayRedirect(order.paymentMethod) &&
        order.paymentStatus === "COMPLETED" ? (
          <>
            {" "}
            Thanh toán trực tuyến đã được xác nhận, đơn đang được xử lý.
          </>
        ) : isOnlineGatewayRedirect(order.paymentMethod) ? (
          <>
            {" "}
            Nếu đã thanh toán, trạng thái sẽ cập nhật trong giây lát.
          </>
        ) : (
          <>
            {" "}
